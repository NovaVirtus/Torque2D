...
class Actor : public SimObject  
{
	...
	public:
		...
		bool moveToPosition(const int logicalX, const int logicalY);

		int mLogicalX;
		int mLogicalY;
		F32 mSpeed;
		U32 mArrivedEventID;
		U32 mContinueMoveCheckEventID;
		...
		ConsoleMethod(Actor, moveToRelativePosition, bool, 4, 4, "(int logicalX, int logicalX) - Moves this actor to the scene at specified location.\n"
			...
			return object->moveToPosition(object->mLogicalX + dAtoi(argv[2]), object->mLogicalY + dAtoi(argv[3]));
